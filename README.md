...
### Q1: 编译时找不到 CloudSim 依赖？

**A**: CloudSim 5.0 从 JitPack 下载，确保网络连接正常。如果下载失败，可以手动下载 `cloudsim5.0.jar` 放到 `libs/` 目录。

### Q2: 多目标算法只返回一个解？

**A**: 检查以下几点：
- 确认使用了 `createCloudlets` 任务生成器（不是 `createCloudlets1`）
- 检查 `ARCHIVE_SIZE` 是否设置足够大（建议 ≥ 200）
- 增加 `POPULATION` 和 `MAX_ITER` 以提高多样性
- 查看 `draw/只有一个解的原因分析.md` 获取详细说明

### Q3: 如何查看详细的算法执行日志？

**A**: 在 `SimulationRunner.java` 中，`CloudSim.init()` 的第三个参数设置为 `true` 可启用详细日志：
```java
CloudSim.init(MainRunner.Config.NUM_USER, Calendar.getInstance(), true);
```

### Q4: 实验结果文件格式是什么？

**A**: 
- CSV 文件使用逗号分隔，UTF-8 编码
- 第一行为表头
- 数值保留 6 位小数
- Pareto 前沿文件包含 4 列：Makespan, CostEfficiency, LoadBalanceIndex, ResourceWaste

### Q5: 如何修改实验配置而不重新编译？

**A**: 实验配置在 `MainRunner.Config` 中，修改后需要重新编译。对于频繁修改的参数，可以考虑使用配置文件或命令行参数。

### Q6: BatchRunner 运行时间很长怎么办？

**A**: 
- 减少 `TRIALS_PER_EXPERIMENT`（如从 10 改为 5）
- 减少任务数量范围（如只测试 100, 300, 500, 1000）
- 减少 `POPULATION` 和 `MAX_ITER`
- 使用更强大的硬件或考虑并行化

### Q7: 如何对比不同实验的结果？

**A**: 
- 使用 `draw/` 目录下的 Python 脚本进行可视化对比
- 查看各 `run_X/summary_avg.csv` 文件进行数值对比
- 使用 `multi_algorithm_comparison.ipynb` 进行综合对比分析
