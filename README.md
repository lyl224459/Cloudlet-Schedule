...
本项目是一个云计算任务调度仿真平台，基于 CloudSim 5.0 框架，实现了多种元启发式优化算法和传统调度算法，用于解决云环境下的任务调度问题。项目支持单目标优化和多目标优化两种场景，可评估不同调度策略在 **完成时间（Makespan）**、**负载均衡（Load Balance）** 和 **成本（Cost）** 等多个维度的性能表现。
...
- ✅ **详细的性能指标**：包括 Makespan、Load Balance、Cost、TotalTime 等
